...
import lmdb
import torch
from torch.utils.data import Dataset, ConcatDataset, sampler, Subset, SubsetRandomSampler
...
class Batch_Mixed_SubDataset(object):
    ...
    def __init__(self, opt, dataset_roots, batch_size, learn_type=None):
        ...
        for root in dataset_roots:
            ...
            if data_type == "label":
                cur_dataset = LmdbDataset(root, opt, mode="train")
                data_usage_ratio = opt.data_usage_ratio
            else:
                cur_dataset = LmdbDataset_unlabel(root, opt)
                data_usage_ratio = opt.unlabel_data_usage_ratio
            dataset_list.append(cur_dataset)
        ...
        number_dataset = int(total_number_dataset * float(data_usage_ratio))
        ...
        partial_dataset_indices, _ = indices[:number_dataset], indices[number_dataset:]
        partial_dataset_labelled = SubsetRandomSampler(partial_dataset_indices)

        data_loader = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=batch_size,
            shuffle=False,
            sampler=partial_dataset_labelled,
            num_workers=num_worker,
            pin_memory=False)
        print("%s dataset length: %d * %d" %
              (learn_type, len(data_loader), batch_size))
