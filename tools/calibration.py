...
def ACE(data, bin_num, correct_fn=None, vis=True, save_pth='reliability_diagrams', prefix='', testing=False):
    ...
    for i in range(bin_num):
        ...
        if correct_fn is None:
            ...
            ds_correct = np.array(list(map(lambda x: int(x[2]==x[3]), ds)))
...
def ECE(data, bin_num, correct_fn=None):
    ...
    confidences = np.array(list(map(lambda x: x[0], data)))
    accuracies = np.array(list(map(lambda x: int(equal(x[2], x[3])), data)))
    ...
    return ece.item(), np.max(bin_score), accuracies.mean()
        ...
        # assert(len(char_conf) == len(pred))
            ...
            for op in op_str:
                if op == 's':
                    gg = gt.pop(0)
                    pp = pred.pop(0)
                    dd = [char_conf.pop(0), 0, gg, pp]
                elif op == '#':
                    gg = gt.pop(0)
                    pp = pred.pop(0)
                    dd = [char_conf.pop(0), 1, gg, pp]
                elif op == 'd':
                    gg = '#'
                    pp = pred.pop(0)
                    dd = [char_conf.pop(0), 0, gg, pp]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json_dir', type=str, default='result/json_for_test', help='Where to load json file')
    parser.add_argument('--rd_output_dir', type=str, default='./Reliability_Diagram')
    parser.add_argument('--save_path', type=str, default='test.xlsx')
