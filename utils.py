...
class CTCLabelConverter(object):
    ...
    def encode(self, word_string, batch_max_length=25):
        ...
        word_index = torch.LongTensor(
            len(word_string), batch_max_length).fill_(self.dict['[PAD]'])
    ...
    def decode(self, word_index, word_length):
        ...
        for idx, length in enumerate(word_length):
            ...
            for i in range(length):
                ...
                if word_idx[i] >= 4 and not (i > 0 and word_idx[i - 1]
                                             == word_idx[i]):
                    char_list.append(self.character[word_idx[i]])
...
class AttnLabelConverter(object):
    ...
    def __init__(self, character):
        ...
        list_special_token = ['[PAD]', '[UNK]', '[SOS]', '[EOS]', ' '
                              ]  # [UNK] for unknown character, ' ' for space.
        list_character = list(character)
    ...
    def encode(self, word_string, batch_max_length=25):
        ...
        word_index = torch.LongTensor(len(word_string), batch_max_length +
                                      1).fill_(self.dict['[PAD]'])
        ...
        for i, word in enumerate(word_string):
            ...
            word_index[i][1:1 + len(word_idx)] = torch.LongTensor(
                word_idx)  # word_index[:, 0] = [SOS] token
...
def adjust_learning_rate(optimizer, iteration, opt):
    ...
    for milestone in opt.schedule:
        lr *= opt.lr_drop_rate if iteration >= (float(milestone) *
                                                opt.num_iter) else 1.
...
def save_exp_config(exp_name, run_code_root):
    exp_name = os.path.join(run_code_root, exp_name)
    if not os.path.exists(exp_name):
        os.mkdir(exp_name)
    org_file_list = []
    org_file_list.extend(glob.glob(sys.path[0] + "/**/*.py", recursive=True))
    org_file_list.extend(glob.glob(sys.path[0] + "/**/*.sh", recursive=True))
    for org_file in org_file_list:
        dst_file = os.path.join(sys.path[0], exp_name,
                                os.path.basename(org_file))
        shutil.copyfile(org_file, dst_file)
    print('save experiment file to %s' % exp_name)